...
public class AddCityActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
       ...
       // Toast.makeText(getBaseContext(), "db size : " + Long.toString(dbSize), Toast.LENGTH_LONG).show();
        ...
        while (nbVilles < dbSizeInt) {
            if (villeBdd.getVilleWithID(i) != null) {
                Ville villeTmpFromBdd = villeBdd.getVilleWithID(i);
                listVilles.add(villeTmpFromBdd.getNom());
                //Toast.makeText(getBaseContext(), villeTmpFromBdd.toString(), Toast.LENGTH_LONG).show();
                nbVilles++;
                i++;
            } else {
                //Toast.makeText(getBaseContext(), "rien à " + Integer.toString(i), Toast.LENGTH_LONG).show();
                i++;
            }
        }
        ...
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
               ...
               // Toast.makeText(getBaseContext(), s + " a envoyer", Toast.LENGTH_LONG).show();
    ...
    public void initAlertDialogSimpleClick() {
        ...
        dialogBoxNewCity.setTitle("Ajouter une ville");
        ...
        dialogBoxNewCity.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (!et.getText().toString().isEmpty()) {
                    //ajouterVilleListeView(et.getText().toString());
                   // Toast.makeText(getBaseContext(), " ok different de vide:" + et.getText().toString() + ".", Toast.LENGTH_LONG).show();
                    Ville villeTmp = new Ville(et.getText().toString());
                    villeBdd.insertVille(villeTmp);
                    updateListFromDB();
                    //Toast.makeText(getBaseContext(), et.getText().toString() + " ajouté en bdd", Toast.LENGTH_LONG).show();
                } else {
        ...
        dialogBoxNewCity.setNegativeButton("Annuler", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                ...
                //Toast.makeText(getBaseContext(), "annule", Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    public void initAlertDialogLongClick(final int id) {
        ...
        final Ville villeTmp = villeBdd.getVilleWithNom(s);
    ...
    public void updateListFromDB() {
        ...
        bdd = villeBdd.getBDD();
        long dbSize = DatabaseUtils.queryNumEntries(bdd, "table_villes");
      //  Toast.makeText(getBaseContext(), "UPDATE DE LA BDD - db size : " + Long.toString(dbSize), Toast.LENGTH_LONG).show();
        int dbSizeInt = (int) dbSize;
        int i = 1;
        int nbVilles = 0;
        listVilles.clear();
